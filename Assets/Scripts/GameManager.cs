...
public class GameManager : MonoBehaviour
{
    ...
    */
    ...
    public void PlayerJoined(string jsonPlayerJoin)
    {
        ...
        GameObject name = Instantiate(playerObject, canvas.transform);
        name.GetComponent<Transform>().position = new Vector3(0, 0, 0);
        name.GetComponent<PlayerScript>().SetName(playerJoin.name);
    ...
    */
    public void PlayerLeft(string playerName)
    {
        ...
        foreach (Transform child in canvas.transform)
        {
            if (child.GetComponent<PlayerScript>().GetName() == playerName)
            {
                Destroy(child.gameObject);
            }
        }
    ...
    */
    public void PlayerControllerState(string jsonControllerState)
    {
        ...
        foreach (Transform child in canvas.transform)
        {
            if (child.GetComponent<PlayerScript>().GetName() == controllerState.name)
            {
                child.GetComponent<PlayerScript>().UpdateXY(controllerState.joystick.x, controllerState.joystick.y);
            }
        }
    }
